...
class AuthenticationService: ObservableObject {
    ...
    @Published var currentUser: User? {
        didSet {
            print("[AuthService]: currentUser updated: \(currentUser?.username ?? "nil")")
        }
    }
        ...
        didSet {
            DispatchQueue.main.async {
                ...
                if self.isAuthenticated != newAuthStatus {
                    ...
                    print("[AuthService]: isAuthenticated changed to: \(self.isAuthenticated)")
                }
            }
    ...
    private init() {
        ...
        print("[AuthService]: Initializing and loading stored tokens.")
        loadStoredTokens()
    }
    ...
    private func performRequest(
        ...
        print("[AuthService]: Attempting \(method.rawValue) request to: \(url.absoluteString)")
        ...
        if requiresAuth {
            guard let token = self.accessToken else {
                print("[AuthService]: Error: Auth required but no access token found for \(url.absoluteString).")
        ...
        if let body = body {
            do {
                ...
                print("[AuthService]: Error: Failed to encode request body for \(url.absoluteString): \(error)")
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("[AuthService]: Error: Invalid response (not HTTPURLResponse) for \(url.absoluteString).")
                throw AuthenticationError.invalidResponse(url)
            }
            print("[AuthService]: \(method.rawValue) to \(url.absoluteString) completed with status: \(httpResponse.statusCode)")
            return (data, httpResponse)
        } catch {
            print("[AuthService]: Error: URLSession request failed for \(url.absoluteString): \(error)")
    ...
    func updateUserPreferences(
            ...
            guard isAuthenticated, let userId = self.currentUserId else {
                print("[AuthService]: User not authenticated or no user ID. Cannot update preferences.")
                throw AuthenticationError.sessionExpired
            }

            print("[AuthService]: Attempting to update preferences for user: \(userId)")
            print("[AuthService]: Data to send: PushEnabled: \(pushEnabled), HourPST: \(notificationHourPST), MinutePST: \(notificationMinutePST)")
            ...
            do {
                ...
                if (200...299).contains(httpResponse.statusCode) {
                    print("[AuthService]: Successfully updated preferences on backend.")
                    ...
                    print("[AuthService]: User profile refreshed after preference update.")
                    ...
                    print("[AuthService]: Error updating preferences: \(errorMessage)")
                    throw AuthenticationError.serverError(statusCode: httpResponse.statusCode, message: errorMessage, httpResponse.url)
                }
            } catch let error as AuthenticationError {
                print("[AuthService]: AuthenticationError during preference update: \(error.localizedDescription)")
                throw error
            } catch {
                print("[AuthService]: Unexpected error during preference update: \(error.localizedDescription)")
    ...
    func register(username: String, email: String, password: String) async throws -> RegisterResponse {
        print("[AuthService]: Attempting registration for username: \(username), email: \(email)")
        ...
        switch httpResponse.statusCode {
            case 201:
                do {
                    ...
                    print("[AuthService]: Success: \(registerResponse.username) registered.")
                    return registerResponse
                } catch {
                    print("[AuthService]: Error: Failed to decode successful registration response: \(error)")
            ...
            case 400:
                ...
                print("[AuthService]: Error 400: \(message)")
            ...
            case 409:
                print("[AuthService]: Error 409: Email or username taken.")
            ...
            default:
                ...
                print("[AuthService]: Error \(httpResponse.statusCode): \(message)")
        ...
        switch httpResponse.statusCode {
            case 200:
                do {
                    ...
                    print("[AuthService]: Token acquisition successful.")
                    ...
                    await MainActor.run {
                        ...
                        if let userId = self.extractUserIdFromToken(loginResponse.access) {
                            ...
                            print("[AuthService]: UserID: \(userId) extracted and saved.")
                        }
                        
                        if self.accessToken != nil {
                            self.isAuthenticated = true
                            print("[AuthService]: isAuthenticated directly set to true on MainActor.")
                        } else {
                            self.isAuthenticated = false
                            print("[AuthService]: WARNING: accessToken was nil after decoding loginResponse. isAuthenticated set to false.")
                        }
                    }
                    return loginResponse
                } catch {
                    print("[AuthService]: Error: Failed to decode successful login response: \(error)")
                    await MainActor.run { self.isAuthenticated = false }
                    throw AuthenticationError.decodingError(error, httpResponse.url)
                }
            case 401:
                print("[AuthService]: Error 401: Invalid credentials.")
                await MainActor.run { self.isAuthenticated = false }
    ...
    func loginAndFetchProfile(email: String, password: String) async throws {
        print("[AuthService]: Initiating login and profile fetch for email: \(email)")
        _ = try await login(email: email, password: password) // login() already handles setting isAuthenticated

        if self.isAuthenticated {
            print("[AuthService]: Login successful (isAuthenticated is true), proceeding to fetch profile and send pending FCM token.")
            // Send any pending FCM token now that user is authenticated
            sendPendingFCMTokenIfNeeded()
            
            do {
                _ = try await fetchUserProfile()
                print("[AuthService]: loginAndFetchProfile: Profile fetch returned. CurrentUser in AuthSvc: \(self.currentUser?.username ?? "nil")")
            } catch {
                print("[AuthService]: ERROR: fetchUserProfile failed AFTER successful login token acquisition: \(error.localizedDescription)")
            ...
            print("[AuthService]: CRITICAL Error: Login attempt finished, but user is NOT authenticated before profile fetch. This indicates an issue in login() state update logic if login didn't throw.")
    ...
    func logout() {
        ...
        print("[AuthService]: Session cleared from memory and Keychain.")
    }
    
    func fetchUserProfile() async throws -> User {
        guard self.isAuthenticated, let userId = self.currentUserId else {
            print("[AuthService]: Pre-check failed: Not authenticated or no user ID. IsAuth: \(isAuthenticated), UserID: \(self.currentUserId ?? "nil"). currentUser will not be set.")
            throw AuthenticationError.sessionExpired
        }
        print("[AuthService]: Attempting for user ID: \(userId). Current self.currentUser before fetch: \(self.currentUser?.username ?? "nil")")
        ...
        switch httpResponse.statusCode {
            case 200:
                do {
                    ...
                    print("[AuthService]: Success: SUCCESSFULLY DECODED User object. Username: \(user.username), Email: \(user.email), ID: \(user.id)")
                    
                    await MainActor.run {
                        print("[AuthService]: PRE-ASSIGN on MainActor. Current self.currentUser?.username: \(self.currentUser?.username ?? "nil")")
                        self.currentUser = user
                        print("[AuthService]: POST-ASSIGN on MainActor. New self.currentUser?.username: \(self.currentUser?.username ?? "nil")")
                    }
                    saveUserToKeychain(user)
                    //printUserObjectDetails(user, context: "ProfileFetch SUCCESS")
                    return user
                } catch {
                    print("[AuthService]: Error: Failed to decode successful profile response (HTTP 200). This is a CRITICAL error if data was expected.")
    ...
    func refreshAccessToken() async {
        guard let currentRefreshToken = self.refreshToken else {
            print("[AuthService]: No refresh token available. Cannot refresh.")
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("[AuthService]: Error: Invalid response type (not HTTPURLResponse). Logging out.")
                await MainActor.run { self.logout() }
                return
            }
            
            guard httpResponse.statusCode == 200 else {
                print("[AuthService]: Error: Refresh request failed with status \(httpResponse.statusCode). Logging out.")
                await MainActor.run { self.logout() }
                return
            }
            
            let newTokens = try jsonDecoder.decode(LoginResponse.self, from: data)
            print("[AuthService]: Success: Tokens refreshed.")
            await MainActor.run {
            ...
            print("[AuthService]: Fetching profile after successful token refresh.")
            _ = try await fetchUserProfile()
            
        } catch {
            print("[AuthService]: Error: Exception during token refresh: \(error.localizedDescription). Logging out.")
            await MainActor.run { self.logout() }
        }
    }
    ...
    private func loadStoredTokens() {
        ...
        print("[AuthService]: Retrieved from Keychain: access token \(access != nil ? "found" : "NOT found"), refresh token \(refresh != nil ? "found" : "NOT found").")
        
        guard let loadedAccessToken = access, let loadedRefreshToken = refresh else {
            print("[AuthService]: No complete token pair in Keychain. CurrentUser will remain nil.")
            Task {
                await MainActor.run {
                    self.isAuthenticated = false
                    self.currentUser = nil
        ...
        if !isTokenExpired(loadedAccessToken) {
            print("[AuthService]: Access token is valid.")
            Task {
                await MainActor.run {
                    ...
                    if let userId = KeychainManager.shared.retrieveUserId() ?? self.extractUserIdFromToken(loadedAccessToken) {
                        self.currentUserId = userId
                        KeychainManager.shared.saveUserId(userId)
                        print("[AuthService]: UserID set to: \(userId).")
                    }
                    self.isAuthenticated = true
                    print("[AuthService]: isAuthenticated set to true. UserID: \(self.currentUserId ?? "nil")")
                }
                if self.isAuthenticated && self.currentUserId != nil {
                    print("[AuthService]: Conditions MET to fetch profile. IsAuth: \(self.isAuthenticated), UserID: \(self.currentUserId!)")
                    do {
                        print("[AuthService]: Attempting to call fetchUserProfile().")
                        _ = try await self.fetchUserProfile()
                        print("[AuthService]: fetchUserProfile() call completed. Check subsequent logs for currentUser content.")
                    } catch {
                        print("[AuthService]: Error calling fetchUserProfile() from loadStoredTokens: \(error.localizedDescription)")
                        await MainActor.run { self.logout() }
                    }
                } else {
                    print("[AuthService]: Conditions NOT MET to fetch profile. IsAuth: \(self.isAuthenticated), UserID: \(self.currentUserId ?? "nil"). currentUser will remain nil from this path.")
                    // Ensure currentUser is nil if profile isn't fetched
                    await MainActor.run {
                        if self.currentUser != nil {
                            self.currentUser = nil
                            print("[AuthService]: currentUser explicitly set to nil as profile fetch was skipped.")
                        }
                    }
                }
            }
    ...
    private func isTokenExpired(_ token: String) -> Bool {
        ...
        guard segments.count == 3,
            ...
            print("[AuthService]: Failed to parse token for expiration check.")
            return true // Treat as expired if unparseable
        }
        let expirationDate = Date(timeIntervalSince1970: exp)
        let isExpired = Date() >= expirationDate
        print("[AuthService]: Token expiration check: \(isExpired ? "EXPIRED" : "VALID"). Expires at: \(expirationDate)")
        return isExpired
    }
    ...
    private func extractUserIdFromToken(_ token: String) -> String? {
        ...
        guard segments.count == 3,
            ...
            print("[AuthService]: Failed to extract UserID (sub claim) from token.")
            return nil
        }
        return userId
    }
    ...
    private func saveUserToKeychain(_ user: User) {
        print("[AuthService]: Attempting to save user profile to Keychain for user: \(user.username)")
        do {
            ...
            if let userString = String(data: userData, encoding: .utf8) {
                ...
                print("[AuthService]: Error: Could not convert user data to string for Keychain saving.")
            }
        } catch {
            print("[AuthService]: Error: Failed to encode or save user profile to Keychain: \(error)")
        }
    }
    
    func loadStoredUser() -> User? {
        print("[AuthService]: Attempting to load stored user profile from Keychain.")
        guard let userString = try? KeychainManager.shared.retrieve(for: "com.uclamoods.currentUser"),
            ...
            print("[AuthService]: No stored user profile string found or failed to convert to data.")
            return nil
        }
        do {
            let user = try jsonDecoder.decode(User.self, from: userData)
            print("[AuthService]: Successfully loaded and decoded stored user: \(user.username)")
            return user
        } catch {
            print("[AuthService]: Error: Failed to decode stored user profile: \(error)")
            if let decodingError = error as? DecodingError { printDecodingErrorDetails(decodingError) }
            return nil
        }
    }
    ...
    func checkAuthenticationStatus() {
        print("[AuthService]: Explicitly checking authentication status.")
        loadStoredTokens()
    }
    ...
    func sendFCMTokenToBackend(fcmToken: String) {
            ...
            guard isAuthenticated, let userId = self.currentUserId else {
                print("[AuthService]: User not authenticated or no user ID. FCM token not sent yet. Token: \(fcmToken)")
            ...
            print("[AuthService]: Attempting to send FCM token to backend for user: \(userId).")
                ...
                do {
                    ...
                    if (200...299).contains(httpResponse.statusCode) {
                        if String(data: data, encoding: .utf8) != nil {
                            print("[AuthService]: Successfully sent FCM token to backend.")
                        } else {
                            print("[AuthService]: Successfully sent FCM token to backend (no response body).")
                        }
                        ...
                        print("[AuthService]: Error sending FCM token: \(message)")
                        // Store the token to retry later if it failed
                        UserDefaults.standard.set(fcmToken, forKey: "pendingFCMToken")
                    }
                } catch {
                    print("[AuthService]: Network or other error sending FCM token: \(error.localizedDescription)")
        ...
        func sendPendingFCMTokenIfNeeded() {
            if let pendingToken = UserDefaults.standard.string(forKey: "pendingFCMToken") {
                print("[AuthService]: Found pending FCM token. Attempting to send.")
