...
// Utility function (keep this at the top)
...
mapRouter.get('/locations/nearby/:lat/:lng', async (req, res) => {
  try {
    ...
    // Get all locations (you might want to add a rough bounding box query for better performance)
...
// 7. POST search locations by name
mapRouter.post('/locations/search', async (req, res) => {
  try {
    ...
    // Build search criteria using regex (since text index might not exist)
...
// 8. GET location types/categories
...
// 9. GET map bounds for all locations
...
// 10. POST calculate route between points
mapRouter.post('/routes/calculate', async (req, res) => {
  try {
    ...
    const calculatedRoute = {
      ...
      estimatedTime: Math.round(totalDistance * 60 / 50), // Assuming 50 km/h average speed
      routePoints
    };
